...
export function ProductImportForm({ template }: { template: string }) {
  ...
  function doPreview() {
    ...
    try {
      ...
      setPreview(objs.slice(0, 10).map((d) => ({ ok: !!d.sku && !!d.name, data: d })));
    } catch (err) {
  ...
  async function doImport() {
    ...
    try {
      ...
      const payload = objs.map((r) => ({
        ...
        defaultPurchasePrice: r.defaultPurchasePrice ? Number(r.defaultPurchasePrice) : null,
        defaultSellingPrice: r.defaultSellingPrice ? Number(r.defaultSellingPrice) : null,
        requiresSerial: (r.requiresSerial ?? 'true').toLowerCase() !== 'false',
        unitOfMeasure: r.unitOfMeasure || 'Nos',
      }));
      const result = await bulkImportProducts({ rows: payload });
      if (!result.ok) {
        setError(result.error.message);
        return;
      }
      alert(`Imported ${result.data.count} products.`);
      router.push('/catalog');
    } finally {
      setBusy(false);
    }
      ...
      <div className="border-line rounded-[6px] border bg-white p-4">
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex gap-2">
            <Button type="button" size="sm" onClick={() => setCsv(template)}>
              Load template
            </Button>
            ...
            <tbody>
              {preview.map((p, i) => (
                <tr key={i} className="border-line border-b last:border-b-0">
                  <td className="mono px-3 py-2">{p.data.sku}</td>
                  <td className="px-3 py-2">{p.data.name}</td>
                  <td className="mono px-3 py-2">{p.data.hsnCode}</td>
                  <td className="mono px-3 py-2">{p.data.gstRate}%</td>
                  <td className="mono px-3 py-2">{p.data.mrp || '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>
