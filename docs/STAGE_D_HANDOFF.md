...
> removed (DEV.77).
>
> **Progress — D.2 ✅ (ran 2026-05-29).** Pre-production security hardening
>
> - spec-sync workflow shipped, all four carried items closed. **F-1**
>   Next.js `14.2.18 → 14.2.35` (exact pin; CVE-2025-29927 + 4 Server-Component
>   DoS highs cleared; 503 unit + 59 verify specs green on the bumped
>   framework). **F-3** login rate-limit + cumulative lockout into `login()` —
>   5/15-min throttle via `peekRateLimit`/`resetRateLimit` on the existing
>   `rate_limit` table + 10/30-min hard lock via new
>   `users.failed_login_attempts` + `users.lockout_until` columns (migration
>   `0016`, applied to staging + prod via the new R17 whitelist-migrate-remove
>   sequence). Generic `"Invalid email or password."` preserved on every
>   failure path. Production smoke (7× `nonexistent@dealerlink.test` against
>   the live login Server Action) → identical generic error each time;
>   response-time pattern shows the rate-limit short-circuit firing at
>   attempt 6. **DEV.73** reserved-slug guard refines `slugSchema` (admin/
>   app/www + infra superset: mail/staging/api/cdn/…). **DEV.79** discovered
> - closed pre-push: removed the `@logtail/pino` worker-thread transport
>   that broke under Next webpack bundling (surfaced as recurring
>   `Cannot find module 'worker.js'` Sentry events attributed to POST
>   /login — login itself was always fine). **DEV.64** closed via Option C:
>   `pnpm sync-spec:{staging,production}` (`scripts/sync-app-spec.mjs`) +
>   R18 runbook. The script preserves DO-derived `ingress:`, overlays
>   committed services/workers/databases/domains, keeps every encrypted
>   `EV[...]` from the live spec, shows a redacted diff, and prompts before
>   apply. Production verified at commit `9c4cc4b`. **Deferred:** the
>   committed `.do/app.production.yaml` has stale
