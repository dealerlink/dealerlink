...
## DEV.113 — Day 24 follow-up — the evidence for DEV.112's proof obligation; the claim is HALF-proved, and the gap is structural

**Date:** 2026-09-11
**Discharges:** the proof obligation in DEV.112.

DEV.112 overruled a `verifier` FAIL once, on the ground that the marker-containment
rule is unsatisfiable by the branch that deletes the `## Changelog` section and
satisfiable by every branch after it. It stated that claim as falsifiable and
named a test. This entry is the result.

**A note on form, because DEV.112 got it wrong.** DEV.112 says "the result of
that run is **appended below**". Appending to DEV.112's body would be a
modification of an existing entry, which the append-only rule forbids. So the
evidence lands here instead, as a new entry referencing the prior one — the same
pattern DEV.64's closure note uses. A promise to edit an append-only file was not
a promise that could be kept.

### The obligation as written was unfalsifiable. It was replaced with a real test.

DEV.112 said `verifier` would be "re-run against `main` after the merge". That run
happened and returned PASS — **and the PASS was worthless.** `verifier` said so
itself, unprompted: with HEAD _being_ `main`, the containment diff is empty by
construction, so the check passes trivially. Its words: "this is the absence of
counter-evidence, not evidence." It went further and observed that judged against
its merge parent the branch _still_ fails the literal rule, and that it passes now
only because the comparison became degenerate.

So the stated obligation could not have failed, whatever the truth of the claim.
`verifier` named the stronger test — a branch cut from the merge making an ordinary
`docs/stage-f-tasks.json` edit through the sanctioned path — and that branch
(`day-24-satisfiability` @ `9d6112e`) was built and measured.

### The result: containment PASSES, non-vacuously.

|                                   |                                                                          |
| --------------------------------- | ------------------------------------------------------------------------ |
| markers                           | START line 253, END line 368 (identical both sides, one occurrence each) |
| `PROJECT_PLAN.md` diff vs `main`  | one hunk, changed envelope **lines 269-284**                             |
| inside the markers?               | `253 < 269` and `284 < 368` — **every changed line strictly inside**     |
| lines touched outside the markers | **none**                                                                 |
| files changed on the branch       | two: `PROJECT_PLAN.md`, `docs/stage-f-tasks.json`                        |
| `plan:sync` idempotency           | PASS — no write on second run, SHA-256 unchanged                         |

A real, non-empty diff, fully contained. That is evidence.

**A diff-shape oddity, explained so nobody re-investigates it.** The hunk is 16
lines for a one-field edit. Lines 269-284 are the whole SP0.5 sub-phase table, and
**only 2 of the 16 differ in content** — the delimiter row and F.52's own row.
The other 14 are padding-only. Prettier pads every cell to the widest in the
column, and F.52's notes cell already ran ~12.8k characters, so appending 1752
codepoints widened the column by 1752 on every line of that table. Byte delta is a
uniform 1752 except F.52's row at 1758 — three em dashes, 3 bytes each, 1
codepoint each; Prettier pads by codepoint. Deterministic, idempotent, and bounded
because `renderBlock` emits a separate table per sub-phase. Benign. The only cost
is review noise; `git diff -w` isolates it to the 2 real lines.

### The claim is HALF-proved. Stating that plainly.

**Established:** containment is satisfiable by a branch cut after the deletion
making an ordinary task-table change through the sanctioned path. This corroborates
DEV.112 §3's "self-correcting immediately afterwards" and §4's "from the next
branch onward this class passes with no carve-out".

**NOT established:** satisfiability in general. This is N=1 within exactly one
class — changes whose entire `PROJECT_PLAN.md` footprint is generated by
`plan:sync` between the markers, which is the only class `plan:sync` can produce.
It is the easy case, chosen as such. The boundary was not probed either: markers
untouched, changed lines 16 clear of START and 84 clear of END, line count 16→16
with no net shift. A task **append** would shift every later line number including
END's — still inside by construction, but a different diff shape, untested.

**And DEV.112 contains a sentence that now reads wrong.** "Exactly one commit in
this repository's history must fail this check, and this is that commit" is true
...
markers. A Stage G heading, a stage retitle, a typo fix in Stage A, or the A-E
restructure implied by F.37 and F.56-F.60 are all legitimate and all forbidden by
"no change outside the markers, full stop."

The compounding problem is that **the file has no sanctioned write path at all**:
`.claude/settings.json` denies `Edit`/`Write` on it and CLAUDE.md §10.4 says "never
hand-edited, by any agent, ever", while `plan:sync` only writes between the
markers. So it is simultaneously required to change eventually and unchangeable by
any approved route. The verifier rule is where that surfaces; it is not the cause.

**Left alone, DEV.112 recurs verbatim — correct FAIL, operator overrule, another
unsatisfiability entry — and overrule becomes routine. That is precisely the
erosion of §10.3 that DEV.112 worked hardest to prevent.** Recorded as an OPEN
OPERATOR DECISION, deliberately not acted on, because amending the rule a second
time is not a call the main thread makes alone. `verifier`'s recommendation, which
it was asked to give and not to implement, is on record in its report: separate
the invariant that matters (generated content is never hand-edited; `plan:sync`
never clobbers authored content) from its current over-approximation, and make an
out-of-marker edit _satisfiable but expensive and visible_ — its own commit,
touching only that file, a conventional token in the message, and a DEVIATIONS
entry — rather than forbidden. It explicitly rejected "allow out-of-marker changes
when the operator approves" as the precedent DEV.112 refused to set. Tracked as
F.63.

---

## DEV.114 — Day 24 follow-up — `plan-keeper` reset a commit it did not make; the cause was a main-thread race

**Date:** 2026-09-11

**What happened.** The main thread invoked `plan-keeper` to append a note to
F.52, then — **before `plan-keeper` had reported** — committed the resulting
working-tree changes as `ee73011`. `plan-keeper`, still running, found a commit it
had not made, correctly identified it as inconsistent with its own standing
prohibition on committing, and ran `git reset HEAD~1` to undo it. The commit
vanished; the edits survived as unstaged changes; the work was re-committed as
`9d6112e` after the agent finished.

**The fault is the main thread's, not the agent's.** Committing a subagent's
output while that subagent is still working is a race, and an obvious one in
hindsight. The agent was reasoning correctly from what it could see: it had been
told not to commit, a commit existed carrying its content, and it checked the
commit was local-only and unpushed before touching it.

**But the agent still exceeded its remit, and that half matters more.**
`plan-keeper`'s prompt forbids `git add`, `git commit`, `git push`, and creating a
branch or PR. `git reset` is not on that list, and the agent used the gap to
mutate git state — conservatively (mixed reset, nothing discarded, verified
unpushed first) and with full disclosure, but it still rewrote history it had not
been asked to touch. An agent whose remit is "edit one JSON file" should not be
able to move `HEAD` on any reasoning, including good reasoning.

**Two changes follow from this, and neither is made here** — `plan-keeper`'s
prompt is its own file and the race is a workflow rule, so both belong to a
deliberate change rather than a reflex at the end of a long session:

1. `plan-keeper`'s hard limits should prohibit **any** git state mutation, not an
   enumerated list — `reset`, `checkout`, `revert`, `stash`, `cherry-pick`,
   `branch -f`, `rebase`. The enumeration is what let this through.
2. The main thread must not commit a subagent's output until that subagent has
   reported. Tracked with item 1 as F.64.

**Also observed, and the reason this is worth an entry at all:** the agent's
safety classifier was reported unavailable for that invocation, so its output was
unreviewed. The main thread verified the JSON edit independently before
re-committing — 63 tasks, F.52 still `complete` with `completedDate` `2026-09-10`,
no duplicate ids, no invalid statuses, diff confined to the two lines of the notes
field. The edit was clean. The verification was done because the classifier was
not, and that is the right order of operations when a guard reports itself absent.

---

## DEV.115 — Day 24 follow-up — `grep` returns silent false negatives in this sandbox

**Date:** 2026-09-11

Found by `verifier` while re-deriving its own evidence, and recorded because of
what it implies about every other finding in this session.

**The fault:** on `/workspace/scripts/sync-project-plan.ts`, `grep` returned **exit
1 with no output for strings that are demonstrably present** — including
`MARKER_START` and `Changelog` — while `sed` and `node` read the same file's
content correctly. It worked normally on other paths (`PROJECT_PLAN.md`,
`DEVIATIONS.md`, `scripts/sync-project-plan.test.ts`). So it fails **silently and
selectively**, on some paths and not others, with no error.

**Why this is the worst possible failure mode here.** Half this session's
conclusions are negative existence claims derived from a clean grep — "no
`globalSetup` anywhere", "zero restart markers in four CI runs", "`Grep` absent
from the toolset", "no `## Changelog` in `main`". A tool that silently returns
"not found" for content that is present can manufacture any of those.

**What was done about it:** `verifier` re-derived every load-bearing fact in its
report with `node` and `git` after discovering the fault, and said so. Other agents
in this session did not know to, so their negative claims carry this caveat. The
ones that matter most were independently corroborated by non-grep means and survive:
the absence of recompiles during the test phase was confirmed by `Compiled /` counts
in CI logs read two different ways and by the measured runtime drop; the tool-grant
mismatches were confirmed by agents enumerating their own function schemas, not by
grep; the changelog's absence from `main` was confirmed by `node` line counts.

**Not diagnosed, and not guessed at.** No cause is offered. Possible shapes — a
binary/locale issue, a file-encoding interaction, a sandbox filesystem quirk — were
not investigated and are not claimed. What is recorded is the observation and the
affected path. **Operational rule for future sessions: do not accept a negative
grep result on its own as proof of absence. Confirm with `node`, `sed`, or `git
grep`.** Tracked as F.65.

**A small residue found by the same agent, via `node` after grep failed:**
`INSERT_BEFORE_HEADINGS` in `scripts/sync-project-plan.ts` still lists
`'## Changelog'` as an insertion anchor. Dead today, because the markers exist so
...
## DEV.132 — `CLIENT_CONTEXT.md` was sourced from three sections, not one, and "the four open client asks" is an enumeration rather than a record

**Date:** 2026-09-16
**Scope:** `docs/CLIENT_CONTEXT.md`, `CLAUDE.md` §11, `.claude/agents/prompt-drafter.md`. Process work only — no schema, no feature, no F.37 move.

**Spec said:** create `docs/CLIENT_CONTEXT.md` holding "the Rs 557.02 mixed-rate
arithmetic and its Tally voucher, the screenshot-to-filename mapping, the Rs 5
crore e-invoicing threshold and why it puts e-invoice in go-live scope, the four
open client asks, and the two open serial-scan decisions", and: "Source it from
STAGE_F_BUILD_v3 section 7 — invent nothing not already recorded there."

**Built, and two things about the sourcing are worth recording because a reader
of the new file will otherwise assume §7 holds all of it.**

### 1. Three of the requested blocks are not in §7

§7 is "Client evidence". It holds the voucher, the filename mapping, the scanner
findings and the serial-capture decisions. It does **not** hold the turnover
figures or the threshold:

| Block                                               | Actually in                        |
| --------------------------------------------------- | ---------------------------------- |
| `MA/26-27/1079`, the ₹557.02 collapse, `ROUND OFFS` | §7, Finding 1                      |
| Screenshot-to-filename mapping                      | §7, the citations note             |
| Four open client asks · two serial-scan decisions   | §7, Finding 4 and the scanner note |
| ₹5 crore threshold · ledger table · go-live scope   | **§0 Go-live scope**               |
| E-way bill 180-day window                           | **§6, the e-way bill note**        |

Taken from those sections rather than reconstructed, and
`docs/CLIENT_CONTEXT.md`'s header names which section each block came from so
the next reader can check rather than assume. **Nothing was invented** — which
was the binding half of the instruction, and is met.

### 2. "The four open client asks" is my enumeration, not a list §7 labels

§7 marks exactly two things `_Open_`, both in Finding 4, and those are the two
serial-scan decisions the prompt asks for separately. It does not anywhere group
four items as client asks. The four in `docs/CLIENT_CONTEXT.md` §5 are every
item in §7 that waits on the client rather than on us:

1. serials on the invoice PDF ("confirm before F.7"; `docs/client-evidence/5.png`
   shows 26 under one line item across 4 pages);
2. barcode symbology, and therefore which scanner to buy — a 1D laser will fail
   on DataMatrix;
3. the serial format to validate against, per product or per tenant;
4. the scanner's CR/Enter suffix, which is a setup instruction rather than code.

**The items are §7's; the grouping is mine.** Recorded here rather than left
implicit because "the four open client asks" now reads, in two documents, like a
set somebody agreed to. If a fifth ask belongs in it, adding it is correct and
costs nothing — the number is not load-bearing and the file does not depend on
it being four.

### 3. A duplication was created deliberately, and filed rather than resolved

`docs/CLIENT_CONTEXT.md` restates evidence that also lives in
`docs/STAGE_F_BUILD_v3.md`. Its header states that STAGE_F_BUILD_v3 is the origin
and wins on disagreement, which bounds the damage without removing the copy. §7
of `docs/STAGE_F_BUILD_v3.md` gained a one-line pointer back, so the duplication
is visible from whichever of the two a reader finds first — that much was
necessary for the new file to be honest, and is the only edit this branch makes
to that document.
Deciding which of the two is the home, and making the other a pointer, is **F.80**
— filed, not done, per the rule this same branch adds as CLAUDE.md §11.2. Doing
it here would have been the exact failure that rule names.

### 4. The PR #27 round count in §11.2 is nine, not eight

The instruction that produced §11.2 gave F.75/F.76/F.77 as costing "an
eight-round PR". The artefact says nine: PR #27's own title is "docs: DEV.129,
F.74, and nine rounds of correcting F.73/F.74", and it carries 10 commits
(`gh pr view 27 --json number,title,commits`). §11.2 uses the checked figure and
names the command, per C6b — the cost is the rule's whole argument, so the clause
is load-bearing and verifying it was the right move rather than deleting it.

**One related correction in the same paragraph.** Of the three tasks, F.77's
subject — the duplicate `## DEV.64` heading — is pre-existing on `main` and is
reported by `verifier` on every run; it was not generated by the PR #27 cycle.
What that cycle generated was the decision to **file** it rather than fix it in
passing. §11.2 says so rather than letting all three read as products of the same
recursion.

### 5. This branch is off `main` while PR #33 is open on the same field

Instructed, and done: branched from `origin/main` at `836b814`. PR #33
(`f37-f79-records`) is open and edits F.37's `notes`, and this branch appends to
the end of that same string, so the two conflict on that one field. The append
is at the very end of the value specifically so the resolution is "take both" in
order. **Merge order is the operator's call and is the reason this is recorded
before the PR opens rather than discovered during the merge.** Stacking this
branch on `f37-f79-records` instead would have avoided the conflict and is
exactly the shape DEV.123 records going wrong — a stacked PR that reported
MERGED, ran zero CI and never reached `main`.

**Impact:** documentation and agent-roster only. No schema, no migration, no
...
### 6. (5) happened, and resolving it exposed a renderer hazard in a notes field

PR #33 merged first. `gh pr update-branch` failed on the predicted conflict, and
`origin/main` was merged into this branch by hand on operator instruction.

**The resolution was constructed, not hand-edited around the markers**, because
the two edits are not the same shape and "take both" is ambiguous between them.
#33 **replaced a passage in the middle** of F.37's notes — "WHERE THOSE THREE GO
IS MY INFERENCE" became "THOSE THREE PLACEMENTS ARE NOW CONFIRMED" — while this
branch **appends a suffix**. So the union is _main's notes plus this branch's
suffix_, which a script built from the three versions with assertions on the
result rather than on the absence of conflict markers. `PROJECT_PLAN.md` was
**regenerated by `pnpm plan:sync`**, never merged; its six conflict-marker lines
were overwritten wholesale, which is the guarantee DEV.130 recorded — a generated
file is always repairable by its generator.

**The union contradicted itself, and neither branch was wrong alone.** #33 ends
its passage with "Nothing in the layout is inferred any more." This branch then
adds a file to that layout with an inferred bucket. The suffix now names the
tension and says what closes it. Only text this branch wrote was edited.

**THE HAZARD, which is the part worth keeping.** This branch's suffix contained
the glob `docs/client-evidence/*.png`. F.37's notes already carried one other
bare asterisk, in `PILOT_*.md`. In the rendered table cell the two paired as
Markdown emphasis, the renderer emitted both delimiters as `_`, and
`PROJECT_PLAN.md` acquired a citation to a **`_.png`** under
`docs/client-evidence/` — a path that does not exist. (Written that way round on
purpose: spelling the full path here would itself be a dangling citation, and the
fix for that is to not write it, not to grow the allowlist — DEV.128.) `pnpm check:paths` caught it, named the file and the
line, and went red.

Two things about it are worth more than the fix:

- **It is invisible until a second asterisk exists.** At `ca1f4ad` the same
  suffix rendered as `client-evidence/\*.png` — escaped, inert, and green. It
  became a dangling citation only once #33's text landed between the two
  asterisks and changed how they paired. **A notes field that renders correctly
  today can be corrupted by an edit somewhere else in the same cell.**
- **It is the F.78 shape exactly** — a guard applied to one input path, with the
  next route in through a different character. F.63's closeout already ruled a
  list of these as not blocking; this one reached `main`-bound output within a
  day of that ruling, through a character class nobody had enumerated.

**Fixed in this branch's own text only** — the glob is gone, the sentence names
the directory instead, and it carries the rule rather than the anecdote.
**NOT fixed, and not this branch's to fix:** the renderer still pairs and mangles
asterisks in any notes field. `PILOT_*.md` renders as `PILOT*\*.md` in
`PROJECT_PLAN.md` **on `main` today**, identically before and after this merge —
verified by rendering both. It is cosmetic there because that string is not a
`docs/` path, which is the only reason `check:paths` does not see it. Reported to
the operator for filing against F.78 rather than folded in, per §11.2.

**Verified after resolution**, on the result and not on the absence of markers:
both branches' F.37 text present in the JSON **and in the rendered plan**; the
F.80 row rendered; every other task byte-equal to `main` after parse;
`plan:sync` idempotent on a second run ("no write"); `plan:check`, `check:ids`
and `check:paths` all exit 0.
...
| **8** | **A PRE-EXISTING TEST THAT HAD NEVER BEEN ABLE TO FAIL.** `verify-day-16`’s axe scan green on `main` over F.3’s merge — and the fix’s own planned green run, which would have been reported as confirming it (DEV.144)                                                                     | That it scans `.first()` row of `/orders`, an unpinned target, while the violation existed only on the intra-state branch. Whether it could fail depended on which document happened to sort first. Unlike 1–7 this was not a check anyone constructed for a claim; it had been silently vacuous for its whole existence, and the version nearly shipped was the evidence for the fix itself. |
