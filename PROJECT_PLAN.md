...
| C.2 | State-code normalization (closes DEV.33)        | ⏳ next |            | Migrate full state names → 2-letter codes across tenant_settings/dealers/quotations; tighten CHECK. Blocks Phase-2 GST Returns API.            |
| C.3 | Pilot staging handoff + UX walkthrough          | ⏳      | 2026-05-24 | Folds old C.1–C.3: provision 2 demo tenants via the admin app, walk every workflow as each role, generate real GST docs vs `3 PO Premier.pdf`. |
| C.4 | Security audit                                  | ⏳      | 2026-05-25 | Folds old C.4: cross-tenant RLS isolation (bypassing the UI), role-enforcement audit, operator read-only impersonation, `/security-review`.    |
| C.5 | Performance test + Stage D handoff              | ⏳      | 2026-05-26 | Folds old C.5–C.6: stress test (10K inventory items, 200 deals, 50 PDFs), capture + fix critical issues, prepare the Stage D handoff.          |

**Stage C status: 🔄 In progress (2/6) — C.0 staging + C.1 force-password-change done; validation (C.2–C.5) pending**
