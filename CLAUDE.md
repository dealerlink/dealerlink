...
| PDF gen    | **Typst** in workers process | ADR-015. Templates in `apps/workers/src/templates-typst/`; hermetic (vendored fonts, no network), byte-deterministic, snapshot-tested. Replaced Puppeteer/Chromium on Day 27 |
...
| Concern              | Pick                                     | Notes                                                                                                                                                                                                                                                                                                                                                                                                                          |
| -------------------- | ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Hosting              | **DigitalOcean App Platform**, Bangalore | Web + workers both on DO App Platform components (BLR1 region). Web: basic-xs (buildpack). Workers: basic-xs in production, basic-xxs in staging. The workers Dockerfile is a plain Node image plus the pinned Typst binary as of Day 27 (ADR-015); it carried Chromium under ADR-013. Production was raised off 512 MB after an OOM under Chromium rendering (DEV.67) — no longer needed, peak RSS is now 119.6 MB (DEV.126). |
...
- ❌ Don't render PDFs on the web process — always queue to the workers process (ADR-013 for the queue-isolation contract; ADR-015 for what runs inside the job).
...
| `docs/PROJECT_HISTORY.md`                                         | **Main thread or operator**            | Stages 0–E, the risk register, the deferred-feature list. Ordinary markdown, edited normally. It exists so `PROJECT_PLAN.md` could become fully generated: the narrative needed a home where editing it is not a rule violation (F.63, DEV.112).                                                                                                                                                                                  |
| `PROJECT_PLAN.md`                                                 | **Generated — nobody**                 | `pnpm plan:sync` renders the WHOLE file from `docs/stage-f-tasks.json` plus the header template `docs/project-plan-header.md`. Never hand-edited, by any agent, ever — and since F.37/F.63 there is no region where a hand edit would be legitimate, so the rule finally matches the file. `pnpm plan:check` fails CI on drift. Stages 0–E moved to `docs/PROJECT_HISTORY.md`, which IS hand-maintained and carries no deny rule. |
...
That last one used to have a known dead zone: the file carried Stage 0 and
Stages A–E outside the `STAGE_F_TASKS` markers, which `plan:sync` could not
write, so the deny forbade the only route to a legitimate change. F.37/F.63
moved that content to `docs/PROJECT_HISTORY.md` and made the whole file
generated, so the deny is now unqualified and CORRECT rather than blunt — there
is nothing it blocks that anyone should be doing.
